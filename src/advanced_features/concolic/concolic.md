...
LibAFL 支持基于 [SymCC](https://github.com/eurecom-s3/symcc) 插桩编译器的协程跟踪。

对于那些没有经验的人来说，下面将尝试用一个例子从头开始描述协程跟踪。
然后，我们将讨论 SymCC 和 LibAFL 协程跟踪的关系。
最后，我们将通过使用 LibAFL 构建一个基本的混合模糊器。
...
并行追踪允许我们以 **分析的方式而不是** 随机的方式 (即猜测) 构建一个输入，以尝试程序中的一个新路径 (比如例子中的崩溃路径) 。
...
我们通过否定每个分支条件并分析解决所产生的 `expression` 来做到这一点。
...
LibAFL中的协程跟踪支持是通过 SymCC 实现的。
SymCC 是 clang 的一个编译器插件，可以用来替代普通的 C 或 C++ 编译器。
SymCC 将对编译后的代码进行回调，使之成为一个可以由用户提供的运行时。
...
根据 SymQEMU 的 [构建说明](https://github.com/eurecom-s3/symqemu#readme) 来构建它。
默认情况下，SymQEMU 会在一个同级目录下寻找运行时。
